...
const useNormalizedConversation = ({
  ...
  const scheduleReconnectRef = useRef<
    ((processId: string, projectId: string) => void) | null
  >(null);
  const establishSSEConnectionRef = useRef<
    ((processId: string, projectId: string) => void) | null
  >(null);
        ...
        fetchEventSource(url, {
          ...
          onopen: async (response) => {
            ...
            if (manager.onopenCalled) {
              ...
              scheduleReconnectRef.current?.(processId, projectId);
              return;
            }
    ...
    (processId: string, projectId: string) => {
      ...
      manager.reconnectTimeout = window.setTimeout(() => {
        if (manager.processStatus === 'running') {
          ...
          establishSSEConnectionRef.current?.(processId, projectId);
        }
      }, delay);
    },
    [debugLog]
  ...
  establishSSEConnectionRef.current = establishSSEConnection;
